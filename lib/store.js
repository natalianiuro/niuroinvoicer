...
const initBankPayments = [
  { id: "p1", beneficiaryName: "Buk",         rut: "77.755.178-7", amount: 290000,  accountNumber: "12345678", bankCode: "001", email: "pagos@buk.cl",        glosa: "Suscripción Buk Marzo 2026",       date: "2026-03-01", groupId: "pg1" },
  { id: "p2", beneficiaryName: "WeWork",       rut: "96.502.060-8", amount: 1200000, accountNumber: "87654321", bankCode: "012", email: "billing@wework.com",   glosa: "Arriendo oficina Marzo 2026",      date: "2026-03-01", groupId: "pg1" },
  { id: "p3", beneficiaryName: "TGP",          rut: "88.123.456-3", amount: 450000,  accountNumber: "11223344", bankCode: "009", email: "admin@tgp.cl",         glosa: "Servicios TGP Marzo 2026",         date: "2026-03-05", groupId: "pg1" },
  { id: "p4", beneficiaryName: "Contabilidad", rut: "12.987.654-1", amount: 380000,  accountNumber: "55667788", bankCode: "001", email: "contador@estudio.cl",  glosa: "Honorarios contabilidad Mar 2026", date: "2026-03-05", groupId: "pg1" },
];

const initPaymentGroups = [
  { id: "pg1", name: "March 2026", month: 3, year: 2026, cuentaOrigen: "" },
];

const initReimbursementGroups = [
  { id: "rg1", name: "March 2026", month: 3, year: 2026, cuentaOrigen: "" },
];

const initOnboarding = [
  {
    id: "ob1", personName: "Harry Potter", role: "QA Engineer", client: "eztax", startDate: "2025-10-14",
    checklist: { contractSigned: true, ndaSigned: true, niuroCredentials: true, niuroOnboarding: true, computer: true, clientCredentials: true },
    notes: "Onboarding complete.",
  },
  {
    id: "ob2", personName: "Ron Weasley", role: "Technical Lead", client: "fundo", startDate: "2025-10-27",
    checklist: { contractSigned: true, ndaSigned: true, niuroCredentials: true, niuroOnboarding: true, computer: true, clientCredentials: false },
    notes: "Client credentials pending.",
  },
  {
    id: "ob3", personName: "Hermione Granger", role: "Full Stack Engineer", client: "bhp", startDate: "2026-01-15",
    checklist: { contractSigned: true, ndaSigned: true, niuroCredentials: true, niuroOnboarding: true, computer: false, clientCredentials: true },
    notes: "No company computer (uses own).",
  },
  {
    id: "ob4", personName: "Draco Malfoy", role: "Full Stack Engineer", client: "tickblaze", startDate: "2026-01-20",
    checklist: { contractSigned: true, niuroCredentials: true, computer: true, niuroOnboarding: true, clientCredentials: true },
    notes: "",
  },
  {
    id: "ob5", personName: "Cho Chang", role: "Full Stack Engineer", client: "cdk", startDate: "2026-04-01",
    checklist: { contractSigned: false, ndaSigned: false, niuroCredentials: false, niuroOnboarding: false, computer: false, clientCredentials: false },
    notes: "Just joined.",
  },
];

const initDevSuccess = [
  { id: "ds1", personName: "Harry Potter",  sentDate: "2026-01-15", type: "check-in",    status: "completed", satisfaction: 4, highlights: "Buena dinámica de equipo", challenges: "Documentación escasa", notes: "" },
  { id: "ds2", personName: "Ron Weasley", sentDate: "2026-01-15", type: "check-in",    status: "completed", satisfaction: 5, highlights: "Autonomía y proyectos interesantes", challenges: "Timezone con cliente", notes: "" },
  { id: "ds3", personName: "Hermione Granger",      sentDate: "2026-02-01", type: "satisfaction", status: "completed", satisfaction: 4, highlights: "Buen ambiente", challenges: "Carga de trabajo alta", notes: "" },
  { id: "ds4", personName: "Draco Malfoy",  sentDate: "2026-02-01", type: "satisfaction", status: "sent",      satisfaction: null, highlights: "", challenges: "", notes: "" },
  { id: "ds5", personName: "Luna Lovegood",  sentDate: "2026-03-01", type: "check-in",    status: "sent",      satisfaction: null, highlights: "", challenges: "", notes: "" },
];

const initPerformanceReviews = [
  { id: "pr1", personName: "Harry Potter",  reviewDate: "2026-04-14", rating: 4, salaryBefore: 4500, salaryAfter: 4800, currency: "USD", notes: "Excelente desempeño. Aumento 6.7%.", nextReviewDate: "2026-10-14" },
  { id: "pr2", personName: "Ron Weasley", reviewDate: "2026-04-27", rating: 5, salaryBefore: 5000, salaryAfter: 5500, currency: "USD", notes: "Superó expectativas. Promotion a Lead.", nextReviewDate: "2026-10-27" },
];
...
const INITIAL_STATE = {
  ...
  paymentGroups: initPaymentGroups,
  reimbursementGroups: initReimbursementGroups,
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case "ADD_PAYMENT_GROUP":    return { ...state, paymentGroups: [...(state.paymentGroups || []), action.payload] };
    case "UPDATE_PAYMENT_GROUP": return { ...state, paymentGroups: (state.paymentGroups || []).map((g) => g.id === action.payload.id ? { ...g, ...action.payload } : g) };
    case "DELETE_PAYMENT_GROUP": return { ...state, paymentGroups: (state.paymentGroups || []).filter((g) => g.id !== action.payload), bankPayments: state.bankPayments.filter((p) => p.groupId !== action.payload) };
    case "ADD_REIMBURSEMENT_GROUP":    return { ...state, reimbursementGroups: [...(state.reimbursementGroups || []), action.payload] };
    case "UPDATE_REIMBURSEMENT_GROUP": return { ...state, reimbursementGroups: (state.reimbursementGroups || []).map((g) => g.id === action.payload.id ? { ...g, ...action.payload } : g) };
    case "DELETE_REIMBURSEMENT_GROUP": return { ...state, reimbursementGroups: (state.reimbursementGroups || []).filter((g) => g.id !== action.payload), reimbursements: state.reimbursements.filter((r) => r.groupId !== action.payload) };
    // People
...
const STORAGE_KEY = "niuro_hr_state_v4";
  ...
  const [state, dispatch] = useReducer(reducer, INITIAL_STATE, (init) => {
    ...
    try {
      ...
      return {
        ...
        paymentGroups:        parsed.paymentGroups        ?? init.paymentGroups,
        reimbursementGroups:  parsed.reimbursementGroups  ?? init.reimbursementGroups,
